...
class MaxHeap {
    ...
    //
    // Example: after extractMax, root might be 5 with children 19 and 22
    //   5 < 19 and 5 < 22, and 22 > 19 → swap with 22, then fix that subtree
    ...
    // Remove and return the maximum value (always at the root, index 0)
    // Steps:
    //   1. Save the root value (the max)
    //   2. Move the last element into the root
    //   3. Decrease size (last slot is no longer part of the heap)
    //   4. Push the new root down with maxHeapify until heap property is restored
    //
    // Example: heap [84, 19, 22, 5, 6, 17]
    //   popped = 84
    //   move last (17) to root → [17, 19, 22, 5, 6], size = 5
    //   maxHeapify(0): 17 < 19 and 17 < 22, 22 is larger → swap with 22
    //                  → [22, 19, 17, 5, 6]
    //   return 84
    public int extractMax() {
        int popped = Heap[0];     // save the maximum value (root)
        Heap[0] = Heap[--size];   // decrease size first, then put last element at root
        maxHeapify(0);            // restore max-heap property from the root downward
        return popped;            // return the removed maximum
    }
}

// Demo program: build a max heap, print it, remove the max, print again
public class MainClass {
    ...
    public static void main(String[] arg) {
        ...
        // extractMax() removes 84 and returns it; print shows the heap after removal
        System.out.println("\nNodes of the max heap after deleting the maximum value "
                + maxHeap.extractMax() + " :");
        maxHeap.print(); // print the heap again (root is now the new maximum)
    }
}
